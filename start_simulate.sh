#!/bin/bash
# AMiGA Simulation Startup Script
...
export AMIGA_SIMULATE=1

# Start backend, capture output for debugging if it fails
python3 -m uvicorn backend.api.main:app --host 0.0.0.0 --port 8000 --reload > backend_sim.log 2>&1 &
BACKEND_PID=$!
sleep 2 # Give it a second to see if it crashes immediately
if ! kill -0 $BACKEND_PID 2>/dev/null; then
    echo "❌ Backend failed to start! Check backend_sim.log for details:"
    cat backend_sim.log
    exit 1
fi
